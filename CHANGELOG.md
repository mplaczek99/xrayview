...
- Analyze overlay rebuilt to produce coherent tooth/bone regions instead of per-pixel speckle. The two forests are now scored into continuous probability maps, those maps are box-blurred (radius `min(w,h)/36`), and every pixel takes the **argmax** of {tooth, bone, background} — mutually exclusive classes — rather than thresholding each forest independently. Smoothing the scores (not the hard masks) turns the isodense per-pixel noise into solid regions resembling the hand-drawn references; the tooth/bone boundary it draws is the alveolar bone level, so the tooth highlight naturally stops where bone begins. A small `+0.05` argmax bias toward tooth keeps isodense roots on the tooth side, and a "both forests below 0.5 ⇒ background" rule restores the dark background the old pipeline flooded over (coverage was ~1.0, now bounded ≤1). The old independent-threshold detectors remain as a fallback if a forest asset fails to load. Pure integral-image work, parallelized over rows — no asset or model-format change.
- Analyze now highlights only the **tooth surface down to the bone level**: the embedded root is reclassified as bone, so green marks the crown and stops at the alveolar crest. The split is a single multi-source flood from the argmax masks — every background pixel seeds a "crown" front and every bone pixel a "root" front, both at distance 0, and each tooth pixel takes whichever front reaches it first (the equidistant locus is the bone level). Background is seeded first so ties keep the pixel as tooth, and with no bone present the whole tooth is kept. Orientation-free (works for single-arch periapicals and both arches of a bitewing) and needs no per-column/edge assumption about where the jaw sits.
