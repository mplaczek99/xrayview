...
This document completes phase 40 from [GO_BACKEND_MIGRATION_PLAN.md](GO_BACKEND_MIGRATION_PLAN.md). The repository no longer treats the Tauri shell as the supported desktop host. The live desktop app is now the Wails shell in `wails-prototype/`, which launches the existing React workstation frontend, owns native shell responsibilities, manages the Go sidecar lifecycle, and forwards the frontend command surface to the backend over the existing local HTTP contract.
...
- [wails-prototype/scripts/run.mjs](wails-prototype/scripts/run.mjs)
...
- the Wails shell binary is built at `wails-prototype/build/bin/xrayview`
- the bundled Go sidecar binary is built beside it at `wails-prototype/build/bin/xrayview-go-backend`
- the React frontend assets are built into `wails-prototype/build/frontend/dist/`
...
- `go-sidecar` starts or attaches to the loopback backend target
